"""Unit tests for makelib.version module."""
...
def test_update_project_version(tmp_path: Path) -> None:
    """Verify update_project_version modifies pyproject.toml and __init__.py."""
    ...
    update_project_version("0.2.0", pyproject_path=fake_pyproject, init_path=fake_init)

    assert 'version = "0.2.0"' in fake_pyproject.read_text(encoding="utf-8")
    assert '__version__ = "0.2.0"' in fake_init.read_text(encoding="utf-8")

    # Non-existent files should be skipped cleanly
    update_project_version(
        "0.3.0",
        pyproject_path=tmp_path / "missing.toml",
        init_path=tmp_path / "missing.py",
    )
